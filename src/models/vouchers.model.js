...
// models/vouchers.model.js (potongan yang diganti)
import { query } from "../configs/db.js";
import {
...
export async function redeemVoucher(customerId, voucherId) {
  ...
  // 4) Buat discount code via GraphQL Discounts API
  const code = `LOYAL-${customerId}-${Date.now().toString().slice(-6)}`;

  // hitung endsAt dari duration_days jika ada
  const startsAt = new Date();
  const endsAt = v.duration_days
    ? new Date(
        startsAt.getTime() + Number(v.duration_days) * 86400000
      ).toISOString()
    : undefined;

  let discountNodeId;
  if (v.discount_type === "percentage" || v.discount_type === "fixed_amount") {
    discountNodeId = await createDiscountCodeBasic({
      code,
    ...
    discountNodeId = await createDiscountCodeFreeShipping({
      code,
      startsAt: startsAt.toISOString(),
      endsAt,
    });
    ...
    const err = new Error("unsupported_discount_type");
    err.status = 400;
    err.details = { got: v.discount_type };
    throw err;
  }

  // 5) Simpan redemption (+ opsional simpan id discount di tabel vouchers)
  await query(
    `INSERT INTO redemptions(customer_id, voucher_id, code, shopify_discount_id)
     VALUES ($1, $2, $3, $4)`,
    [customerId, v.id, code, discountNodeId]
  );

  // (opsional) kalau kamu ingin simpan di vouchers agar reusable:
  // await query(`UPDATE vouchers SET shopify_discount_id=$1 WHERE id=$2`, [discountNodeId, v.id]);

  return { voucher: v, code, shopify_discount_id: discountNodeId };
}
